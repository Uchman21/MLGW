...
class RNN_walk():
    ...
    def sample_random_neighbor(self, current_x):
        ...
        neighbors_weight = tf.realdiv(mask, tf.reduce_sum(mask, -1, keepdims=True))
    ...
    def __cost(self, trueX):
        ...
        with tf.variable_scope("cost", reuse=tf.AUTO_REUSE):
            ...
            #
            # M_loss = tf.reduce_mean(KL, 1)#tf.reduce_mean(tensor_without_nans, axis=1)
    ...
    def train(self, labels, train_ratio, masks, nepochs=100, verbose=True, save=False):
        ...
        with tf.Session(config=config) as sess:
            ...
            if save:
                ...
                # saver.save(sess, '{}/model'.format(self.save_path))
                np.save('path/{}_idx2'.format(self._dataset), train_mask)
    ...
    def get_emb(self, sess, X, largebatchsize=200):
        ...
        np.save("path/{}_emb2".format(self._dataset), emb)
        # try:
    ...
    def __save_path(self, sess, X, largebatchsize=200):
        ...
        np.save("path/{}_node2".format(self._dataset), path_node)
        # try:
        #     shutil.rmtree(self.save_path)
        # except OSError as e:
        #     print ("Error: %s - %s." % (e.filename, e.strerror))
