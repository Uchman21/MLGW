...
class RNN_walk():
    ...
    def sample_random_neighbor(self, current_x):
        ...
        neighbors_weight = tf.div_no_nan(mask, tf.reduce_sum(mask, -1, keepdims=True))
    ...
    def sample_neighbor_walk(self, current_x, current_emb, h, t, reuse=tf.AUTO_REUSE):
        ...
        # neighbor_node_emb = tf.nn.embedding_lookup(self.node_emb, tf.gather(self.edge_tensor, neighbors))
        #
        # neighbors_weight = tf.keras.backend.hard_sigmoid(self.dense(att_emb, 1, name='e_e_dense2_private', is_private=True ))
        ...
        neighbors_weight = tf.squeeze(tf.keras.backend.hard_sigmoid(self.dense(att_emb, 1, name='e_e_dense2_private', is_private=True )),-1)
        ...
        # neighbors_weight = tf.squeeze(self.dense(att_emb, 1, name='e_e_dense2_private', is_private=True ),-1)
        #
        #
        # att_emb_glob = self.dense(att_emb, self.config['l_dim'], name='e_e_dense2_emb', is_private= True)
        # neighbors_weight_glob = tf.keras.backend.hard_sigmoid(self.dense(att_emb_glob, 1, name='e_e_dense2_global', is_private=False ))
        # # neighbors_weight_glob = tf.squeeze(self.dense(att_emb_glob, 1, name='e_e_dense2_global', is_private=False ),-1)
        #
        # # concat_weight = tf.concat((neighbors_weight, neighbors_weight_glob), -1)
        # # weight_avg = tf.nn.softmax(self.dense(concat_weight, 2, name='weight_avg', is_private=True), axis=-1)
        # # final_weight = tf.reduce_sum(tf.multiply(concat_weight, weight_avg), -1)
        # # final_weight = tf.squeeze(tf.realdiv(tf.add(neighbors_weight, neighbors_weight_glob), 2), -1)
        # # final_weight = tf.squeeze(neighbors_weight, -1)
        # final_weight = tf.squeeze(tf.multiply(neighbors_weight, neighbors_weight_glob), -1)
        #
        # neighbors_weight_glob = tf.multiply(neighbors_weight_glob,tf.cast(tf.expand_dims(mask,-1), tf.float32))
        # neighbors_weight = tf.multiply(neighbors_weight,tf.cast(tf.expand_dims(mask,-1), tf.float32))
        # final_weight = tf.multiply(final_weight,tf.cast(mask, tf.float32))
        #
        # neighbors_weight_glob = tf.squeeze(neighbors_weight_glob, -1)
        # neighbors_weight = tf.squeeze(neighbors_weight, -1)
        #
        # filter_neighbors = tf.greater_equal(final_weight, 0.5)
        # mask2 = tf.logical_and(filter_neighbors, mask)
        #
        # neighbors_weight_glob = tf.div_no_nan(neighbors_weight_glob , tf.reduce_sum(neighbors_weight_glob, -1, keepdims=True))
        # global_policy = tf.distributions.Categorical(logits=tf.log(tf.clip_by_value(neighbors_weight_glob,1e-10,1.0)))
        ...
        # private_policy = tf.distributions.Categorical(logits=tf.log(tf.clip_by_value(neighbors_weight,1e-10,1.0)))
        #
        # final_weight = tf.div_no_nan(final_weight , tf.reduce_sum(final_weight, -1, keepdims=True))
        # joint_policy = tf.distributions.Categorical(logits=tf.log(tf.clip_by_value(final_weight,1e-10,1.0)))
        #
        #
        # # filter_neighbors = tf.greater_equal(neighbors_weight, 0.5)
        # # mask2 = tf.logical_and(filter_neighbors, mask)
        # # neighbors_weight = tf.multiply(neighbors_weight, tf.cast(mask2, tf.float32))
        # # neighbors_weight = tf.realdiv(neighbors_weight , tf.reduce_sum(neighbors_weight, -1, keepdims=True))
        #
        # # private_policy = tf.distributions.Categorical(logits=tf.log(tf.clip_by_value(neighbors_weight,1e-10,1.0)))
        # # next_id_sample = private_policy.sample()
        ...
        # likelihood = tf.squeeze(tf.batch_gather(final_weight, next_id_sample),[-1])
        # likelihood = tf.multiply(tf.pow(self.config['gamma'], (self.T - t)), tf.ones_like(tf.log(tf.clip_by_value(likelihood,1e-10,1.0))))
        # KL = tf.distributions.kl_divergence(private_policy, global_policy)
        #
        ...
        # neighbors_weight_glob = tf.squeeze(tf.keras.backend.hard_sigmoid(self.dense(att_emb_glob, 1, name='e_e_dense2_public', is_private=False )),-1)
        # neighbors_weight_glob = tf.multiply(neighbors_weight_glob,tf.cast(mask, tf.float32))
        # neighbors_weight_glob = tf.realdiv(neighbors_weight_glob , tf.reduce_sum(neighbors_weight_glob, -1, keepdims=True))
        # global_policy = tf.distributions.Categorical(logits=tf.log(tf.clip_by_value(neighbors_weight_glob,1e-10,1.0)))

        # likelihood = tf.squeeze(self.gather_colsv2(neighbors_weight, next_id_sample),[-1])#tf.add(tf.squeeze(self.gather_colsv2(neighbors_weight, next_id_sample),[-1]) , tf.cast(~non_isolated_nodes, tf.float32))
        likelihood = tf.squeeze(tf.batch_gather(final_weight, next_id_sample),[-1])
        likelihood = tf.multiply(tf.pow(self.config['gamma'], (self.T - t)), tf.ones_like(tf.log(tf.clip_by_value(likelihood,1e-10,1.0))))
        KL = self.config['kl_cost'] * tf.distributions.kl_divergence(private_policy, global_policy)
        entropy = self.config['en_cost'] * tf.reduce_mean(tf.losses.log_loss(labels=neighbors_weight, predictions=neighbors_weight_glob, reduction=tf.losses.Reduction.NONE), -1)

        return tf.expand_dims(next_id,-1), neighbor_emb, tf.expand_dims(likelihood,-1), tf.expand_dims(tf.add(KL, entropy),-1)
        # else:
        #     return tf.expand_dims(next_id,-1), neighbor_emb
    ...
    def GRU(self, trueX):
        ...
        def forward(input, t):
            ...
            next_x, x_t_e, likelihood, KL = self.sample_neighbor_walk(x, x_t, h_tm1, t)
    ...
    def __cost(self, trueX):
        ...
        with tf.variable_scope("cost", reuse=tf.AUTO_REUSE):
            ...
            # Z = tf.nn.relu(self.dense(Z, self.config['l_dim'], name='Z2Z'))
            ...
            # tensor_without_nans = tf.where(tf.is_nan(KL), tf.zeros_like(KL), KL)
            #
            # M_loss = tf.reduce_mean(_cost, 1)#tf.reduce_mean(tensor_without_nans, axis=1)

            M_loss = tf.reduce_mean(_cost, 1) - tf.reduce_mean(reward * likelihood, 1) + tf.reduce_mean(KL, 1)
            return tf.reduce_mean(tf.reduce_mean(M_loss, -1))
...
# model = edge_att(ref_features.shape[0])