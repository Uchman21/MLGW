...
def parse_args():
	...
	flags.DEFINE_float('kl_cost',0.1, 'KL divergance cost penalty. Used only for mlgw_kl variant.')
 	flags.DEFINE_integer('dimensions', 128, 'Dimension size of the latent vectors. Default is 128.')
	...
	flags.DEFINE_integer('num-walks', 10,'Number of walks per source. Default is 10.')
	flags.DEFINE_integer('num-length',20,'Number of nodes to tranverse per walk. Default is 20.')
...
def benchmark_ml(FLAGS):
	...
	cv_splits, Y, blacklist_samples = iterative_sampling(labels.toarray(), labeled_idx, patritions, rng)
	...
	for i in range(1):  #change range(1) to range(len(cv_splits)) to evaluate on all splits
		...
		#train and evaluate
		acc, score_micro, score_macro, per_label =  r_walk.train(labels, samples, False)
